/* icons.jsx — Bonsai brand mark + Lucide-style line icon set (1.6px stroke) */

// Brand mark: a multi-canopy bonsai with spreading roots — Bonsai.
// Recreated from the reference in tonal blues. viewBox 0 0 134 126.
function BonsaiMark({ size = 24, style }) {
  const deep = "#0A1E47";     // roots, branches, darkest canopies
  const trunk = "#FFFFFF";    // main trunk only
  ...
  const s = size / 134;
  return (
    <svg width={size} height={size * (126 / 134)} viewBox="0 0 134 126" fill="none" style={style} aria-hidden="true">
      {/* spreading roots / base */}
      ...
      <g stroke={deep} fill="none" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* left branch from base, with twig */}
...
// Full lockup
function BonsaiLogo({ height = 22, color = "var(--fg)" }) {
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 9 }}>
      <span style={{ width: height + 12, height: height + 12, borderRadius: 11, background: "var(--tile-bg)", border: "1px solid var(--line-2)", display: "grid", placeItems: "center" }}>
        <BonsaiMark size={height + 2} />
      </span>
      <span style={{ display: "flex", flexDirection: "column", lineHeight: 1 }}>
        <span style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: height * 0.82, letterSpacing: "-0.02em", color }}>
          Bons<span style={{ color: "var(--accent)" }}>ai</span>
        </span>
        <span style={{ fontSize: 9.5, letterSpacing: "0.22em", textTransform: "uppercase", color: "var(--fg-3)", marginTop: 3, fontWeight: 600 }}>
          by Agent Studio
        </span>
      </span>
    </span>
...
Object.assign(window, { BonsaiMark, BonsaiLogo, Icon, ICON_NAMES_LIST: Object.keys(ICON_PATHS) });
